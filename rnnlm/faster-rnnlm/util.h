...
// 在 IRecWeights 类中使用 （interface.h）
...
class WeightMatrixUpdater {
 public:
  explicit WeightMatrixUpdater(Matrix* weights)
    : weights_(*weights)
    , gradients_(weights_.rows(), weights_.cols())   // 类似对 Matrix 类进行初始化
    , mean_squared_gradients_(weights_.rows(), weights_.cols())  // 类似对 Matrix 类进行初始化
  {
    gradients_.setZero();
