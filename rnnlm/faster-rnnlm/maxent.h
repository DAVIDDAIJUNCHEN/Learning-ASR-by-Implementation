...
int CalculateMaxentHashIndices(
    const WordIndex *sen, int word_index, // [A, B, C, D ,E]  p(C|h)
...
// 给定句子数组 sen 和 词的位置，计算 ngram 的 hash 值
// maxent_order = 3, sen = [A, B, C, D, E, F], wid = 3, sen[wid] = D
// Ngram: D, (C, D), (B, C,  D)
// ngram_hashes = [g(D, x ), g([C, D], x), g([B, C, D], x)]
// P(D|h) = 
inline int CalculateMaxentHashIndices(
      const WordIndex *sen, int word_index,int maxent_order, uint64_t max_hash,
      bool add_padding, uint64_t ngram_hashes[MAX_NGRAM_ORDER]) {
