...
static void ReadHeader(FILE* file, NNetConfig* cfg, int* version_ptr) {
  ...
  cfg->nce_lnz = 9;  // magic value for default lnz in old versions （提前定义好的默认值，可配置）
  if (version == 0) {
    cfg->use_nce = false;
  } else if (version == 1) {
    cfg->use_nce = true;
...
// 利用 ReaderHeader 从 model_file 获取 cfg 的如下配置参数
// layer_size; maxent_hash_size; maxent_order; nce_lnz; user_nce; reverse_sentence;
// layer_type; layer_count; hs_arity;
...
NNet::NNet(const Vocabulary& vocab, const std::string& model_file, bool use_cuda,
           bool use_cuda_memory_efficient)
    : cfg(ReadConfig(model_file))  // 调用 ReadConfig 函数
    , vocab(vocab)
    , rec_layer(NULL)
    , nce(NULL)
    , use_cuda(use_cuda)
...
// 对 syn_rec_ 进行对角初始化 (alpha*W^rec)
...
void NNet::Save(const std::string& model_file) const {
  if (!cfg.use_nce && !cfg.reverse_sentence && cfg.hs_arity == 2 &&
  ...
  rec_layer->GetWeights()->Dump(file); //调用了 IRecWeights 的 Dump 方法；
