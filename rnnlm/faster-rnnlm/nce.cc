...
int NCE::DetectEffectiveMaxentOrder(
  ...
  for (size_t order = 0; order < maxent_size; ++order) {
    ...
    if (maxent->IsNull(maxent_index)) // 检测是否<10^{-6}
      return order;
  }
  return maxent_size;
}
...
void NCE::Updater::PropagateForwardAndBackward(
  ...
  for (int idx = 0; idx < sample.size + 1; idx++) {
    ...
    } // 更新 embedding[i] 以及 hidden_grad[i]
...
void NCE::CalculateLog10ProbabilityBatch(
    ...
    const WordIndex* sentence, int sentence_length, const bool do_not_normalize,
    ...
    , noise_min_cells_(noise_min_cells)
{
  ...
  if (std::numeric_limits<WordIndex>::max() <= vocab_size) {
    ...
    fprintf(stderr, "ERROR Vocabulary size is too big for UnigramNoiseGenerator\n");
    exit(1);
  }
  ...
  // 考虑 addon 的情形， quasi_freqs 加上 addon, 然后累计
  if (addon > 0) {
...
uint64_t UnigramNoiseGenerator::PrepareNoiseSample(
  ...
  if (n_samples > kMaxNoiseSamples) {
    fprintf(stderr, "ERROR: Cannot use more than %d noise samples!\n", kMaxNoiseSamples);
    exit(1);
  }

  for (int i = 0; i < n_samples; ++i) {
    uint32_t random_value = GetNextRandom(&random_state);  // 在 util.h 中定义
...
uint64_t HSMaxEntNoiseGenerator::PrepareNoiseSample(
  ...
  int maxent_present = CalculateMaxentHashIndices(
      ...
      kMaxentAddPadding, ngram_hashes); // 返回对应 sen_pos 的 ngram_hash 数组

  for (int i = 0; i < n_samples; ++i) {
    tree_->SampleWord(
        ngram_hashes, maxent_present, NULL, maxent_layer_,
