...
struct NoiseSample {
  WordIndex noise_words[kMaxNoiseSamples];  // [w1, w2, w3,...]
  Real noise_ln_probabilities[kMaxNoiseSamples]; // ln[p_n(w1), p_n(w2), p_n(w3),...]
  Real target_ln_probability;               // ln(p_nce(w_target))
  int size;
};
...
class NCE {
 public:
    ...
    NCE(bool use_cuda, bool use_cuda_memory_efficient,
    ...
    // Calculate unnormalized ln probability of a word
