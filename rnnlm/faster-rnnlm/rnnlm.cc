...
void TrainLM(
  ...
  if (nnet->cfg.use_nce) {
    if (nce_maxent_model_weight_file[0] != 0) {
      ...
      // ?? 为什么 layer_size 必须为 0 才能初始化 HSMaxEntNoiseGenerator()
  ...
  bool do_lr_decay = false;  // 初始状态不做 learning rate 衰减
  ...
  for (int n_bad_epochs = 0, epoch = 1; n_bad_epochs <= max_bad_epochs; ++epoch) {
    ...
    if (std::isnan(entropy) || std::isinf(entropy) || !(ratio >= bad_ratio)) {
      ...
      // 经过一轮训练之后，entropy 不合法或者增加了太多就拒绝更新模型
...
void SampleFromLM(NNet* nnet, const std::string& sample_file, int seed, int n_samples, Real generate_temperature) {

  // 读入文件内容
  bool kAutoInsertUnk = false;
  SentenceReader reader(nnet->vocab, sample_file, nnet->cfg.reverse_sentence, kAutoInsertUnk);
  ...
  while (reader.Read()){
    ...
    //   printf("wid: %d, word: %s; ", wids[i], nnet->vocab.GetWordByIndex(wids[i]));
    // }
    // printf("\n");
      ...
      for (; sen.back() != 0;) {
  ...
  
        {
          ...
          for (wid = 0; p >= 0 && wid < nnet->vocab.size(); ++wid) {
            p -= probs[order[wid]];
          }    // [0, 1, ..., vocab_size] ==> [p(0), p(1), |..., p(vocab_size)]
          wid -= 1; // sum[0, wid] <= p, sum[0, wid+1] >p
          sen.back() = order[wid];
...
int main(int argc, char **argv) {
  ...
  int layer_count = 1;
  std::string model_vocab_file, test_file, sample_file, train_file, valid_file;
  ...
  opts.Add("sample", "Sample file with begining of sentences", &sample_file);
  ...
  opts.Add("nce", "Number of noise samples; if nce is positive then NCE is used instead of HS", &nce_samples);
  ...
  opts.Add("nce-maxent-model", "Use the given model as a noise generator; the model must be a pure maxent model trained by the program", &nce_maxent_model_weight_file);
  opts.Echo();
  opts.Echo();
  opts.Echo("How to");
  ...
  opts.Echo("  sample model:");
  opts.Echo("    ./rnnlm -rnnlm model_name -sample sample_beg.txt -generate-samples 2");
  opts.Echo();
  ...
  if (bad_ratio < awful_ratio) {  // bad_ratio = reject-threshold && awful_ratio = stop ratio
  ...
  if (n_samples > 0) {
    ...
    if (!sample_file.empty()) {
        SampleFromLM(main_nnet, sample_file, random_seed, n_samples, generate_temperature); 
    } else {
        fprintf(stderr, "ERROR sample file must be included in sample mode\n");
    }
